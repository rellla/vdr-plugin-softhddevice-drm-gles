...
int cAlsaDevice::GetAvailableBufferFrames(bool sync)
{
	...
	if (availableFrames == -EAGAIN) {
		...
		if (sync)
			LOGWARNING("audio: %s: snd_pcm_avail() failed: %s", __FUNCTION__, snd_strerror(availableFrames));
		else
			LOGWARNING("audio: %s: snd_pcm_avail_update() failed: %s", __FUNCTION__, snd_strerror(availableFrames));
	}
