...
std::vector<uint8_t> createMpeg2PesPacket() {
    auto data = createBasicPesVideoHeader(0xE0, true); // Video stream 0
...
std::vector<uint8_t> createAudioPesPacket() {
    auto data = createBasicPesVideoHeader(0xC0, false); // Audio stream 0
...
TEST_CASE("cPesVideo - Basic construction", "[pes]") {
    SECTION("Construct with valid data") {
        auto data = createBasicPesVideoHeader(0xE0);
...
TEST_CASE("cPesVideo - Stream type detection", "[pes]") {
    SECTION("Video stream detection") {
        ...
        for (uint8_t id = 0xE0; id <= 0xEF; id++) {
            auto data = createBasicPesVideoHeader(id);
            cPesVideo pes(data.data(), data.size());
    ...
    SECTION("Neither audio nor video") {
        auto data = createBasicPesVideoHeader(0xBD); // Private stream 1
        cPesVideo pes(data.data(), data.size());
...
TEST_CASE("cPesVideo - MPEG2 codec detection", "[pes]") {
...
TEST_CASE("cPesVideo - H.264 codec detection", "[pes]") {
...
TEST_CASE("cPesVideo - HEVC codec detection", "[pes]") {
    SECTION("Detect HEVC codec without leading zero") {
        auto data = createHevcPesVideoPacket(false);
        cPesVideo pes(data.data(), data.size());
    ...
    SECTION("Detect HEVC codec with leading zero") {
        auto data = createHevcPesVideoPacket(true);
        cPesVideo pes(data.data(), data.size());
...
TEST_CASE("cPesVideo - PTS handling", "[pes]") {
    SECTION("Get PTS from packet with PTS") {
        auto data = createBasicPesVideoHeader(0xE0, true);
        cPesVideo pes(data.data(), data.size());
    ...
    SECTION("Get PTS from packet without PTS") {
        auto data = createBasicPesVideoHeader(0xE0, false);
        cPesVideo pes(data.data(), data.size());
...
TEST_CASE("cPesVideo - to_string utility", "[pes]") {
